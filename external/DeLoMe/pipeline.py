...
def _legacy_task_seq(args, n_cls):
    return [
        list(range(i, i + int(args.n_cls_per_task)))
        for i in range(0, n_cls - 1, int(args.n_cls_per_task))
    ]


def resolve_task_seq(args, n_cls):
    task_seq_file = getattr(args, 'task_seq_file', None)
    if not task_seq_file:
        return _legacy_task_seq(args, n_cls)

    with open(task_seq_file, 'r', encoding='utf-8') as f:
        payload = json.load(f)
    task_seq = payload.get('class_splits', payload) if isinstance(payload, dict) else payload

    if not isinstance(task_seq, list) or not task_seq:
        raise ValueError('task_seq_file must contain a non-empty list of class splits')

    normalized = []
    seen = set()
    for task_id, task_cls in enumerate(task_seq):
        ...
        for cls_id in current:
            if cls_id < 0 or cls_id >= n_cls:
                raise ValueError(f'class id {cls_id} out of range for n_cls={n_cls}')
            if cls_id in seen:
                raise ValueError(f'class id {cls_id} is duplicated in task_seq_file')
            seen.add(cls_id)
        normalized.append(current)

    missing = sorted(set(range(n_cls)) - seen)
    if missing:
        raise ValueError(f'task_seq_file does not cover all classes, missing: {missing}')
    return normalized
...
def data_prepare(args):
    ...
    cls = resolve_task_seq(args, args.n_cls)
    args.task_seq = cls
...
def pipeline_task_IL_no_inter_edge(args, valid=False):
    ...
    dataset = NodeLevelDataset(args.dataset,ratio_valid_test=args.ratio_valid_test,args=args)
    args.d_data, args.n_cls = dataset.d_data, dataset.n_cls
    cls = resolve_task_seq(args, args.n_cls) # this line will remove the final task if only one class included
    args.task_seq = cls
    args.n_tasks = len(args.task_seq)
    ...
    for task, task_cls in enumerate(args.task_seq):
        ...
        for t in range(task + 1):
            subgraph, ids_per_cls, [train_ids, valid_ids_, test_ids_] = pickle.load(open(f'{args.data_path}/{str_int_tsk}/{args.dataset}_{args.task_seq[t]}.pkl','rb'))
...
def pipeline_task_IL_no_inter_edge_minibatch(args, valid=False):
    ...
    for task, task_cls in enumerate(args.task_seq):
        ...
        save_model_path = f'{args.result_path}/{subfolder_c}val_models/{save_model_name}.pkl'
        n_cls_so_far += len(task_cls)
        subgraph, ids_per_cls, [train_ids, valid_idx, test_ids] = pickle.load(open(f'{args.data_path}/{str_int_tsk}/{args.dataset}_{task_cls}.pkl', 'rb'))
        features, labels = subgraph.srcdata['feat'], subgraph.dstdata['label'].squeeze()
        task_manager.add_task(task, n_cls_so_far)
