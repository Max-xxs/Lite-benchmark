...
class NodeLevelDataset(incremental_graph_trans_):
    def __init__(self,name='ogbn-arxiv',IL='class',default_split=False,ratio_valid_test=None,args=None):
        ...
        if name[0:4] == 'ogbn':
            ...
            data = RedditDataset(raw_dir=args.ori_data_path, self_loop=False)
            graph, label = data.graph, data.labels.view(-1, 1)
        elif name in ['coauthor-cs', 'Coauthor-CS', 'CoauthorCS', 'CoauthorCS-CL']:
            data = CoauthorCSDataset(raw_dir=args.ori_data_path)
            graph, label = data[0], data[0].ndata['label'].view(-1, 1)
        elif name in ['amazon-computers', 'Amazon-Computers', 'AmazonComputers', 'AmazonComputers-CL']:
