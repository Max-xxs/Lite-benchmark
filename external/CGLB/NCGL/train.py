...
if __name__ == '__main__':
    ...
    parser.add_argument('--task_seq_name', type=str, default=None,
                        help='Optional short tag used in result filenames')
    ...
    for hyp_params in hyp_param_list:
        ...
        task_descriptor = args.n_cls_per_task
        if args.task_seq_file:
            task_descriptor = args.task_seq_name or os.path.splitext(os.path.basename(args.task_seq_file))[0]
        name = f'{subfolder}val_{args.dataset}_{task_descriptor}_{args.method}_{list(hyp_params.values())}_{args.backbone}_{backbone_args[args.backbone]}_{args.classifier_increase}_{args.cls_balance}_{args.epochs}_{args.repeats}'
        if args.minibatch:
        ...
        if os.path.isfile(f'{args.result_path}/{name}.pkl') and not args.overwrite_result:
            ...
            for ite in range(args.repeats):
                ...
                try:
                    ...
                    if ite > 0:
                        name_ = f'{subfolder}val_{args.dataset}_{task_descriptor}_{args.method}_{list(hyp_params.values())}_{args.backbone}_{backbone_args[args.backbone]}_{args.classifier_increase}_{args.cls_balance}_{args.epochs}_{ite}'
